...
// Comprehensive Lead Card Modal - Full details view
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Lead Profile - {lead.name || lead.phone}</DialogTitle>
        </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a href={`tel:${lead.phone}`} className="font-mono text-blue-600 hover:text-blue-700 font-medium">
                        {lead.phone}
                      </a>
                    </div>
                    {leadDetails?.email && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-sm text-gray-700">{leadDetails.email}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Badge className={`mt-1 ${
                      lead.stage === "Customer" ? "bg-green-600" :
                      lead.stage === "Interested" || lead.stage === "Qualified" ? "bg-blue-600" :
                      lead.stage === "Not interested" || lead.stage === "Junk" ? "bg-red-600" :
                      "bg-gray-600"
                    }`}>
                    ...
                    {events.map((event, index) => (
                      <div key={index} className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                        <div className={`w-3 h-3 rounded-full mt-1.5 flex-shrink-0 ${
                          ...
                          event.type === "CALL_COMPLETED" ? "bg-green-500" :
                          event.type === "NOTE_ADDED" ? "bg-purple-500" :
                          "bg-gray-400"
                        }`} />
                        <div className="flex-1 min-w-0">
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {event.type === "STAGE_CHANGE" && (() => {
                                ...
                                if (actorId && actorId !== "system") {
                                  return `${resolveActorName(actorId)} changed stage`;
                                }
                          ...
                          {event.type === "STAGE_CHANGE" && event.data?.from && event.data?.to && (
                            <div className="flex items-center gap-2 text-sm mb-1">
                              <Badge variant="outline" className="text-xs">{event.data.from}</Badge>
                              <span className="text-gray-400">→</span>
                              <Badge variant="outline" className="text-xs">{event.data.to}</Badge>
                            </div>
                          )}
                          
                          {event.data?.message && (
                            <p className="text-sm text-gray-600 mt-1">{event.data.message}</p>
                          )}
                          
                          {event.data?.stageNotes && (
                            <div className="mt-2 p-2 bg-blue-50 border-l-2 border-blue-400 rounded">
                              <p className="text-sm text-gray-700 italic">"{event.data.stageNotes}"</p>
                            </div>
                          )}
                        </div>
                      </div>
...
function ActivityLogModal({ 
  ...
  useEffect(() => {
    if (lead && isOpen) {
      ...
      setStageNotes("");
      fetchLeadDetails();
      fetchUserNames(); // Always fetch user names to show in timeline
    }
  }, [lead, isOpen]);
          ...
          {showTimeline && events.length > 0 && (
            <div className="space-y-4 mt-6">
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {events.map((event, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border hover:bg-gray-100 transition-colors">
